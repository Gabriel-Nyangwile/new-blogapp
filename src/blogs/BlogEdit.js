...
const BlogEdit = () => {
  ...
  return (
    <div className="w-full relative overflow-hidden  bg-gray-400 border-none py-10">
      <h1 className="text-blue-500 text-5xl bold text-center mb-10">
        Edit Blog
      </h1>
      ...
      >
        ...
        <div className="text-black-500 text-3xl bold 1/3 px-2">
          ...
          {blog.image && (
            <img
              src={blog.image}
              alt={blog.title}
              className="object-cover h-24 w-24 rounded-full overflow-hidden border"
            />
          )}
        </div>
        <div className="text-black-500 text-3xl bold">
        ...
        <section className="w-auto flex flex-row items-center justify-evenly border-none p-4">
          {error && <p className="text-red-500">{error}</p>}
          {user && user.id === blog.authorId && (
            <div>
              <Button
                type="submit"
                className="bg-green-500 hover:bg-green-100 text-white hover:text-black m-10"
              >
                Modifiez le Blog
              </Button>
              <Button
                type="submit"
                className="bg-red-500 hover:bg-red-100 text-white m-10 hover:text-black"
          ...
          <div>
            <Button
              className="bg-blue-500 hover:bg-blue-100 text-white hover:text-black font-bold py-2 px-4 m-10 rounded"
              onClick={() => navigate(`/`)}
            >
              Retour à l'accueil
            </Button>
