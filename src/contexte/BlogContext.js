...
import { addDoc, arrayUnion, collection, deleteDoc, doc, getDocs, increment, Timestamp, updateDoc } from 'firebase/firestore';
...
export const BlogProvider = ({ children }) => {
  ...
  const CreateBlog = async (title, content, imageFile) => {
    try {
      ...
      if (imageFile) {
        const storageRef = ref(storage, `blogs/${blogRef.id}/${imageFile.name}`);
      ...
      setBlogs(prevBlogs => [
        ...prevBlogs, 
        { id: blogRef.id, title, content, image: imageUrl, createdAt: Timestamp.fromDate(new Date()), authorName: user.displayName || user.email, likes: 0, comments: [] }]);
  ...
  const UpdateBlog = async (id, title, content, imageFile) => {
    try {
      ...
      setBlogs(prevBlogs => prevBlogs.map(blog => blog.id === id ? { ...blog, ...updateData } : blog));
    } catch (error) {
  ...
  const addComment = async (blogId, comment) => {
    ...
    try {
      ...
      setBlogs(prevBlogs => prevBlogs.map(blog => blog.id === blogId ? { ...blog, comments: [...(blog.comments || []), comment] } : blog));
    } catch (error) {
  ...
  return (
    <BlogContext.Provider value={{ blogs, setBlogs, CreateBlog, ReadBlog, UpdateBlog, DeleteBlog, addLike, addComment, liked,setLiked }}>
      {children}
